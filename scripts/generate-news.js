...
async function generateNews() {
  ...
  const prompt = `
    ...
    3. Escreve uma notícia completa e factual OBRIGATÓRIAMENTE EM PORTUGUÊS DE PORTUGAL. É MUITO IMPORTANTE que uses as tuas próprias palavras e que o léxico, a gramática e as expressões sejam estritamente de Portugal (ex: "equipa" em vez de "equipe", "golo" em vez de "gol", "guarda-redes" em vez de "goleiro", "ecrã" em vez de "tela"). Reescreve a informação de forma original, criativa e num tom de jornalismo desportivo cativante. NÃO copies as frases da fonte original - cria um texto 100% único e adaptado ao público português.
  ...
  try {
    ...
    const metadata = {
      ...
      fileName: fileName,
      image: imageUrl,
      url: `https://sports-news-theta.vercel.app/news/${fileName.replace('.mdx', '')}`
    };
